...
public class Dtspb5064RollbackMigrationHandler implements MigrationHandler {
    ...
    static final String ROLLBACK_SUMMARY =
        "DTSPB-5064 - Rollback migrate cases in 'Caveat Not Matched' state to 'Caveat Resolution'";
    static final String ROLLBACK_DESCRIPTION =
        "Rollback migrate cases in 'Caveat Not Matched' state to 'Caveat Resolution'";
    ...
    public boolean shouldMigrateCase(
        ...
        final boolean hasCaveatNotMatched = caseData.containsKey(STATE);
        final boolean isInCaveatNotMatchedState = isInCaveatNotMatchedState(caseData);
        if (!hasCaveatNotMatched && !isInCaveatNotMatchedState) {
