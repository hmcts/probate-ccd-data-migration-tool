...
class ReimplMigrationRunnerTest {
    ...
    void runMigrationFailsToStartEvent() {
        ...
        final MigrationHandler migrationHandlerMock = mock();
        when(migrationHandlerMock.startEventForCase(eq(caseSummary), eq(userToken), eq(s2sToken)))
    ...
    void runMigrationShouldNotMigrate() {
        ...
        when(migrationHandlerMock.startEventForCase(eq(caseSummary), eq(userToken), eq(s2sToken)))
                .thenReturn(migrationEvent);
        when(migrationHandlerMock.shouldMigrateCase(eq(migrationEvent)))
                .thenReturn(false);
    ...
    void runMigrationMigrationFailed() {
        ...
        when(migrationHandlerMock.shouldMigrateCase(eq(migrationEvent)))
                .thenReturn(true);
        when(migrationHandlerMock.migrate(eq(migrationEvent)))
                .thenReturn(false);
